    ...
    <aside aria-label="Validation Copilot Chat" className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div
          ...
          className={`bg-white border border-slate-700/80 shadow-2xl rounded-2xl overflow-hidden transition-all duration-300 mb-3 flex flex-col text-slate-100 ${
            isMinimized
              ? 'w-80 h-14'
        ...
        >
          {/* Header */}
          <header className="px-4 py-3 bg-slate-950/80 border-b border-slate-200 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-indigo-600/90 text-white flex items-center justify-center shadow-2xs">
          ...
          {!isMinimized && (
            <>
              ...
              {/* Quick Prompts Chips */}
              <div className="px-3 py-2 bg-slate-950/40 border-t border-slate-200/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
              ...
              <form
                ...
                className="p-3 bg-slate-950/90 border-t border-slate-200 flex items-center gap-2"
              >
                <input
                  type="text"
                  ...
                  disabled={isLoading}
                  className="flex-1 bg-white border border-slate-700 rounded-xl px-3 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
                <button
                  type="submit"
                  disabled={!input.trim() || isLoading}
                  className="p-2 bg-indigo-600 hover:bg-indigo-600-hover disabled:opacity-40 text-white rounded-xl transition-all shrink-0 cursor-pointer shadow-2xs"
                >
      ...
      {!isOpen && (
        <button
          ...
          className="group relative flex items-center gap-2.5 px-4 py-3 bg-indigo-600 hover:bg-indigo-600-hover text-white rounded-full shadow-xl shadow-indigo-600/30 hover:shadow-indigo-600/50 transition-all duration-200 cursor-pointer active:scale-95"
