...
import { HybridOnboardingChat } from "./HybridOnboardingChat";
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {isRunningPipeline ? (
            ...
            <HybridOnboardingChat
              onDone={async (ctx: any) => {
                setIsRunningPipeline(true);
                setCurrentStepIndex(0);
                try {
                  const res = await fetch("/api/validate-idea", {
                    ...
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      ideaTitle: ideaTitle || ctx.testing_context?.round_goal?.primary || "Untitled",
                      targetIcp: targetIcp || ctx.testing_context?.freelancer_requirements?.special_instructions || "",
                      coreProblem: coreProblem || ctx.summaryDraft || "",
                      targetPrice: parseInt(targetPrice) || 249,
                      testing_context: ctx.testing_context,
                      product_model: (globalThis as any).__productModel,
                      roundGoal: ctx.testing_context?.round_goal?.primary,
                    }),
                  });
                  if (res.ok) {
                    const ws = await res.json();
                    setTimeout(()=> { setIsRunningPipeline(false); onWorkspaceCreated(ws); onShowToast("Test Ready", "Your validation is ready to share", "success"); onClose(); }, 800);
                    return;
                  }
                } catch {}
                completePipelineFallback();
